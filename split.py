...
def wordsplit(mydict, f, s):
    a1 = fmm(mydict, s)
    reverse_a2 = rmm(mydict, s)
    a2 = []
    for i in xrange(len(reverse_a2) - 1, -1, -1):
        a2.append(reverse_a2[i])
    ...
    if i < len(a1) and index == 1:
        a.extend(a1[i:])
    if i < len(a2) and index == 2:
        a.extend(a2[i:])
...
def choosechunk(c1, c2, f):
    ...
    average_l1 = float(total_l1) / len(c1)
    average_l2 = float(total_l2) / len(c2)
    #print("average length1: %f, average length2: %f" % (average_l1, average_l2))
